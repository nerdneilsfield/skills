...
- Keep implementation, earned permanent tests, and documentation for the same behavior together. Do not split by extension, file type, Conventional Commit type, or workflow track. Different modules usually separate only when their changes serve independent purposes; a cross-module feature may be one unit. Never stage throwaway checks from `/tmp` or an uncommitted local directory.
