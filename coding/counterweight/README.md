...
For implementation work in a Git repository, all tracks default to local commits after verification. Explicit no-commit instructions and host/repository restrictions take precedence. Commit scope excludes unrelated user changes; push, PR creation, merge, and release require their own authorization. See [commit contract](references/commits.md).
