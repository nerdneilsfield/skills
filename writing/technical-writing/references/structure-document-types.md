...
**Writing:** 区分事实、决策、假设和备选方案。图与正文使用同一名称；验证项对应需求。验证部分说明如何证明设计满足需求，不自行扩展为项目审批、发布准入或汇报制度。

**Common failure:** 只有方案描述，没有目标、边界、失败模式或验证；把未决定事项写成既定事实；用多级 gate、停止上线条款和汇报禁令代替技术设计。
